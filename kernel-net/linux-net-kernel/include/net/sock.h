 ...
 * 到后备队列中        内核调度优先级参考:
 * http://blog.csdn.net/allen6268198/article/details/7567679
 */
...
/*
执行该函数sk_wake_async(将SIGIO或SIGURG信号发送给该套接字上的进程,这是异步I/O机制)
的地方有sk_send_sigurg(接收到带外数据)，sock_def_write_space和sk_stream_write_space
(发送缓冲区发生变化)，有新的数据到来(sock_def_readable)
