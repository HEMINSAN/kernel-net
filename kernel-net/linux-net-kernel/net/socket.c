...
//这里面会创建struct socket结构，然后会调用sock_register(&netlink_family_ops)
//注册的netlink_create来完成struct netlink_sock的创建
			 ...
			 struct socket **res, int kern)
{
	...
	//如果是应用程序创建的是netlink类型的套接字，
	//则会执行 netlink_family_ops 中的 netlink_create
	err = pf->create(net, sock, protocol, kern);  
	//IPV4 PF_INET协议族为 inet_create
	if (err < 0)
		goto out_module_put;
...
/*
sys_socket 应用层创建套接字，走到该系统调用函数  这里的protocol
值为IPPROTO_IP  应用程序直接填的0，就是因为IPPROTO_IP为0
*/
//最终等于 asmlinkage long sys_socket(int family, int type, int protocol)
SYSCALL_DEFINE3(socket, int, family, int, type, int, protocol)
{
	...
	//这里把创建套接字的sock_type屏蔽掉，屏蔽之后的flags为fd--file使用 sock_map_fd
	...
	//创建struct socket和struct sock结构
	retval = sock_create(family, type, protocol, &sock); 
	if (retval < 0)
		goto out;

    //获取一个未被使用的文件描述符，并且申请并初始化对应的file{}结构。
    //将socket与虚拟文件系统retval绑定,该返回值就是应用程序的socket函数的返回值
 ...
 */
//最终等于 asmlinkage long sys_recvfrom(int fd, void __user *ubuf, size_t size, 
//		unsigned flags, struct sockaddr __user *addr, int __user *addr_len)
...
SYSCALL_DEFINE2(socketcall, int, call, unsigned long __user *, args)
{
	...
	case SYS_SOCKETPAIR:
		...
		break;
	case SYS_SEND:
		err = sys_send(a0, (void __user *)a1, a[2], a[3]);	// SYSCALL_DEFINE4
		break;
	case SYS_SENDTO:
		err = sys_sendto(a0, (void __user *)a1, a[2], a[3],
				 (struct sockaddr __user *)a[4], a[5]);	// SYSCALL_DEFINE6
		break;
	case SYS_RECV:
