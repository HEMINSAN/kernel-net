			    ...
			    int fclone, int node)
{
	 ...
	 */
	memset(skb, 0, offsetof(struct sk_buff, tail));	// init 0 to those members before tail in struct sk_buff
 ...
 */
/* 
克隆只是分配SKB，并增加数据缓冲区的引用计数     。
	skb_clone 一般只修改数据SKB描述符，
		新的skb的data指针和skb data指针指向同一个data，skb结构是新的skb。
	pskb_copy 的使用场合一般是既要修改SKB描述符，也要修改数据缓冲区部分，
		所有SKB和data区要同时复制
一般在以下情况下使用 skb_clone:
	当指需要修改skb结构部分，而不需要修改data数据
	(包括二 三层等，也包括data数据payload部分)的时候用这个
...
struct sk_buff *skb_clone(struct sk_buff *skb, gfp_t gfp_mask) //注意skb_clone和pskb_copy()与skb_copy()的区别
{
	...
	n = skb + 1;//子SKB
	/* 
	判断原始skb是否是从 skbuff_fclone_cache 缓冲区中分配的，
	从 skbuff_fclone_cache 分配将预先为clone的skb分配好内存，
	同时判定该预先分配的 clone skb 是否被使用 
	*/
	if (skb->fclone == SKB_FCLONE_ORIG &&
	    n->fclone == SKB_FCLONE_UNAVAILABLE) { 
	    //表面是从skbuff_fclone_cache中分配的SKB,子SKB的data区是空的
		//N+1刚好对应skbuff_fclone_cache中分配空间的最后四字节，用来表示应用技术
		atomic_t *fclone_ref = (atomic_t *) (n + 1);
		/* 置clone的skb中fclone值为SKB_FCLONE_CLONE ，标明其数据区指向原始skb同一数据区 */
